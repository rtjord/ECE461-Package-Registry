...
describe("E2E Test for Package Retrieve Endpoint", () => {
    ...
    it("should return a 200 status for a package uploaded via Content", async () => {
        const response = await axios.get(`${baseUrl}/package/${content_id}`);
    ...
    it("should return a 200 status for a package uploaded via URL", async () => {
        const response = await axios.get(`${baseUrl}/package/${url_id}`);
        expect(response.status).toBe(200);
    ...
    it("should return a 404 status if the package does not exist", async () => {
        try {
            await axios.get(`${baseUrl}/package/non-existant-id`);
        } catch (error) {
