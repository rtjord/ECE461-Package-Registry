...
export async function createKeywordIndex(domainEndpoint: string, indexName: string) {
  try {
    const mapping = {
      mappings: {
        properties: {
...
export async function createTextIndex(domainEndpoint: string, indexName: string) {
  try {
    const mapping = {
      mappings: {
        properties: {
          content: {
            type: "text", // Treat README content as a single string
          },
          timestamp: {
            type: "date", // ISO-8601 date format
          },
